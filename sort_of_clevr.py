...
class Dataset(object):
    ...
    def __init__(self, ids, path, name='default',
                 max_examples=None, is_train=True,batch_size=100,is_loadImage=False):
        self._ids = list(ids)
        self.name = name
        ...
        self.maxGrups = int(len(self.ids) / batch_size)
        #self.maxGrups =10
        count = 1
        while(count<self.maxGrups+1):
           self.batch.append(self.get_split(count,batch_size))
           count = count+1
    def updateIDS(self,ids):
        self._ids = list(ids)
        self.maxGrups = int(len(self.ids) /  self.batch_size)
        #self.maxGrups =10
        count = 1
        self.batch=[]
        while(count<self.maxGrups+1):
           self.batch.append(self.get_split(count,self.batch_size))
           count = count+1
    ...
    def get_split(self, step,batch_size):
        img=[]
        q=[]
        ids=[]
        a=[]
        ...
        ult = batch_size*step
        inicio = ult - batch_size  
        if (ult > len(self._ids)): 
            stepunicos = int(len(len(self._ids))/batch_size)
